...
//    Start!!
    ...
    while (i) {
        ...
        if (fr4g_tp.getHitPoints() > 0 && fr4g_tp.getHitPoints() > 0 && (i == 2 || i == 9 || i == 16)) {
            sc4v_tp.rangedAttack(fr4g_tp.getName());
            fr4g_tp.takeDamage(sc4v_tp.getRangedAttackDamage());
        }

        if (sc4v_tp.getHitPoints() > 0 && fr4g_tp.getHitPoints() > 0 && (i == 3 || i == 10 || i == 17)) {
            fr4g_tp.meleeAttack(sc4v_tp.getName());
            sc4v_tp.takeDamage(fr4g_tp.getMeleeAttackDamage());
        }

        if (fr4g_tp.getHitPoints() > 0 && fr4g_tp.getHitPoints() > 0 && (i == 4 || i == 11 || i == 18)) {
            sc4v_tp.meleeAttack(fr4g_tp.getName());
            fr4g_tp.takeDamage(sc4v_tp.getMeleeAttackDamage());
        }

        if (sc4v_tp.getHitPoints() > 0 && fr4g_tp.getHitPoints() > 0 && (i == 5 || i == 12 || i == 19)) {
            fr4g_tp.repair(50);
            sc4v_tp.repair(50);
        }

        if (sc4v_tp.getHitPoints() > 0 && fr4g_tp.getHitPoints() > 0 && (i == 6 || i == 13 || i == 20)) {
            cout << fr4g_tp.getName() << ": Activating VaultHunter.EXE" << endl;
            sc4v_tp.takeDamage(fr4g_tp.vaultHunter_dot_exe(sc4v_tp.getName()));
        }

        if (sc4v_tp.getHitPoints() > 0 && fr4g_tp.getHitPoints() > 0 && (i == 7 || i == 14 || i == 21)) {
            cout << sc4v_tp.getName() << ": A new Challenger I see!" << endl;
            fr4g_tp.takeDamage(sc4v_tp.challengeNewcomer(fr4g_tp.getName()));
        }
